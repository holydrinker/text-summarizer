...
class Summarizer:
    ...
    def _gen_centroid_query_based(self, query_based_token):
        seen_token = [token.lower() for token in query_based_token if not self.lookup_table.unseen(token)]
        unseen_token = [token.lower() for token in query_based_token if self.lookup_table.unseen(token)]
...
class ErrorMessage:
    ...
    def generate_query_based_warning(unseen_token, seen_token):
        return u"Non è stato possibile utilizzare i termini: " + u", ".join(unseen_token) + u"\n" + \
